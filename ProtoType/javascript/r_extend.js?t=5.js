...
/*
		if(scrolltop > 200 && !is_mobile){
			$(".intro_video").get(0).play();
		}

		var video2 = $(".video_2").offset().top;
		if(scrolltop > video2 - winheight){
			$(".video_2").get(0).play();
		};
		
		var video3 = $(".video_3").offset().top;
		if(scrolltop > video3 - winheight){
			$(".video_3").get(0).play();
		};
*/
		...
		var overviewTop = $("#overview").offset().top-40;
		...
		}else if(scrolltop >= recordingTop && detailTop > scrolltop){
			$(".hd nav > ul > li > a").removeClass("active");
